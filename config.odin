...
// Tunable constants for the whole game. Values are frame counts at a
// locked 60fps, not seconds.
...
// Radius within which a lander will consider a human as a search target;
// shrinks each wave. See human_search_radius() in entity.odin.
...
// End-of-wave human bonus tally: surviving humans are counted up one at a
// time on the level-finish screen, each awarding 100 x wave number points
// (capped at wave 5, so 500/human).
...
// Intro screen sequence: Williams logo flash -> "presents" colour-cycles
// in -> DEFENDER title shatters into place with a colour-cycling
// highlight -> control instructions.
INTRO_WILLIAMS_FLASH_FRAMES :: 12 // 0.2s per colour
INTRO_PRESENTS_AT :: 120 // 2s
INTRO_TITLE_AT :: 240 // 4s
INTRO_TEXT_AT :: 360 // 6s
INTRO_DISPERSE_START :: 30
INTRO_DISPERSE_RATE :: 0.25 // ~2s (120 frames) to fully converge
INTRO_PRESENTS_CYCLE_STEPS :: 30 // 0.5s period
INTRO_HILITE_CYCLE_STEPS :: 6 // 0.1s period
INTRO_WILLIAMS_Y :: f32(100)
INTRO_PRESENTS_Y :: f32(220)
INTRO_TITLE_Y :: f32(400)
INTRO_TEXT_Y :: f32(HEIGHT) - 100
...
// Pool capacities, sized from the level-6 worst case -- everything lives
// in fixed arrays; nothing grows during play.
MAX_ENTITIES :: 128
